...
"""
def loadDataSet():
    return [[1, 3, 4,3,3,3,3,3], [2, 3, 5], [1, 2, 3, 5, 6], [2, 5], [1, 3, 5]]
...
Returns:
    L - 符合条件的元素
    supportData - 符合条件的元素及其支持率组成的字典,也包括一些非频繁元素集
...
def apriori(dataSet, minSupport=0.5):
    ...
    #dataset中的每个元素是一条购买记录，set一下是为了防止一个人买了几种同样的物品
    D = list(map(set, dataSet))
    # print(D)
    # [{1, 3, 4}, {2, 3, 5}, {1, 2, 3, 5, 6}, {2, 5}, {1, 3, 5}]
...
def generateRules(L, supportData, minConf=0.7):
    ...
    for i in range(1, len(L)):
        for freqSet in L[i]:
            ...
            # 因为要计算每个频繁项集中 各个元素之间是否有规则，因此需要创建单个元素组成的集合
            # 比如freqSet是{3,5}，H1就是{3}、{5}，之后计算5->3、3->5
...
if __name__ == '__main__':
    dataSet = loadDataSet()
    # L是频繁项集，supportData是对应的字典，其中包括一些非频繁的集合
